...
# 10-1 LLM Benchmark default preset (standalone)
#
# Fixed behavior:
#   - Skip optional nvidia-jetpack meta-package installation
#   - Use cache mode 1 (remove model and JIT caches after the run)
...
usage() {
  cat <<'EOF'
Usage: run_10_1_LLMBenchmark_default.sh [--prepare] [--status]
...
Environment overrides:
  ...
  MLC_CSV_SOURCE, DUT_NAME, SWAP_FILE,
  NAS_MOUNT_POINT, NAS_MLC_DIR, NAS_MLC_IMAGE_BACKUP,
  MLC_RUNTIME_IMAGE, TEGRATS_INTERVAL_MS

Fixed preset:
  INSTALL_JETPACK=0, MLC_CACHE_MODE=remove, MLC_MODEL_COUNT=12
  R39_MLC_STACK=legacy
EOF
}
...
build_r39_legacy_mlc_image() {
  ...
  if [[ ! -f "$R39_LEGACY_DOCKERFILE" ]]; then
    mkdir -p "$STATE_DIR"
    R39_LEGACY_DOCKERFILE="${STATE_DIR}/Dockerfile.r39_mlc_legacy.embedded"
    cat >"$R39_LEGACY_DOCKERFILE" <<'DOCKERFILE'
# syntax=docker/dockerfile:1

# Embedded R39 compatibility wrapper for the original JetPack R36 MLC stack.
ARG BASE_IMAGE=dustynv/mlc:0.1.4-r36.4.2
FROM ${BASE_IMAGE}

ENV LD_LIBRARY_PATH=/usr/local/lib/python3.10/dist-packages/tvm:/usr/local/cuda/lib64:/usr/lib/aarch64-linux-gnu

LABEL org.max-test-script.r39-compat="legacy-mlc-0.1.3-tvm-0.18.1-q4f16_ft"
DOCKERFILE
    info "Created the embedded R39 legacy Dockerfile: $R39_LEGACY_DOCKERFILE"
  fi
